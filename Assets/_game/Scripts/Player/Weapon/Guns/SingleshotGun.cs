...
public class SingleshotGun : Gun
{
    ...
    void RaycastSegment(Vector3 start, Vector3 end, Bullet bullet)
    {
        ...
        for (int i = 0; i < hits.Length; i++)
        {
            ...
            {
                Debug.Log("Headshot");
                hit.collider.gameObject.transform.root.GetComponentInChildren<PlayerController>()?.GetComponent<IDamageable>()?.TakeDamage(((GunInfo)itemInfo).head);
                break;
            } else if (hit.collider.gameObject.name == "LowerSpine")
            {
                Debug.Log("LowerTorso Shot");
                hit.collider.gameObject.transform.root.GetComponentInChildren<PlayerController>()?.GetComponent<IDamageable>()?.TakeDamage(((GunInfo)itemInfo).lowerTorso);
                break;
            } else if (hit.collider.gameObject.name == "UpperSpine")
            {
                Debug.Log("UpperTorso Shot");
                hit.collider.gameObject.transform.root.GetComponentInChildren<PlayerController>()?.GetComponent<IDamageable>()?.TakeDamage(((GunInfo)itemInfo).upperTorso);
                break;
            ...
            {
                Debug.Log("Limb Shot");
                hit.collider.gameObject.transform.root.GetComponentInChildren<PlayerController>()?.GetComponent<IDamageable>()?.TakeDamage(((GunInfo)itemInfo).limb);
                break;
            }
            else
            {
